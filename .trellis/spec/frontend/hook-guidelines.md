...
| Theme | system preference | `harnessdeck.theme` |

两者通过 `localStorage` 在页面刷新后保持。

## 副作用规则

- `localStorage` 读写封装在 hook 内部，组件不直接操作偏好 key。
...
useEffect(() => {
  listPractices().then(setPractices);
}, []);
```

如果多个视图共享相同的数据加载逻辑，可以提取为专用 hook（如 `useLoopSummary`），但不强制要求。简单的 `useState` + `useEffect` 组合在视图内部使用即可。
...
Hook 通过组件测试覆盖。复杂 hook 可以使用 `@testing-library/react` 的 `renderHook` 单独测试。
