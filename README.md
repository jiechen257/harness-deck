...
当前实现为本地优先 fixture mode。应用默认显示简体中文和浅色主题，支持英文和深色切换。主窗口对齐 command deck 原型，包含顶部命令栏、北斗品牌状态带、菜单栏面板和 macOS 窗口化工作台。工作台包含首页、发现、配置集、同步、运行、用量、洞察、守护和设置；独立 Tauri 菜单栏面板通过 `index.html?panel=1` 渲染，展示当前配置集、同步健康度、成本、防睡状态和快捷动作。
