...
Phase 0 + Phase 1 采用 **北斗导航 + 司南工程仪表** 作为视觉方向。

要求：

- 定义品牌 token：北辰蓝、玉衡青、丹朱金、赤霞红、天青紫、星图白、玄夜蓝黑。
- 菜单栏面板和工作台使用不同 surface 层级，避免整套 UI 只有一种背景色。
- 生命周期导航显示七个北斗节点：Discover、Profile、Sync、Operate、Usage、Insight、Guard。
- 北斗节点只作为导航和状态符号使用，避免大面积装饰和插画化。
