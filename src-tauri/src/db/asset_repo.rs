...
impl Database {
    ...
    pub fn list_assets(&self) -> Result<Vec<LocalAsset>, CommandError> {
        ...
        let rows = stmt.query_map([], |row| Ok(LocalAsset {
            id: row.get(0)?,
            practice_id: row.get(1)?,
            asset_type: row.get(2)?,
            registry_path: row.get(3)?,
    ...
    pub fn list_assets_by_practice(&self, practice_id: &str) -> Result<Vec<LocalAsset>, CommandError> {
        let mut stmt = self.conn().prepare(
        ...
        let rows = stmt.query_map(params![practice_id], |row| Ok(LocalAsset {
            id: row.get(0)?,
            practice_id: row.get(1)?,
            asset_type: row.get(2)?,
            registry_path: row.get(3)?,
