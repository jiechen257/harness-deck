...
mod tests {
    ...
    #[test]
    fn schema_records_latest_migration_version() {
        let db = test_db();
        let version: i32 = db
            .conn()
    ...
    fn signal_crud() {
        ...
        db.update_signal_status(&signal.id, "normalized")
    ...
    fn ops_script_crud() {
        let db = test_db();
        let script = db
            .insert_ops_script(&NewOpsScript {
    ...
    fn audit_event_crud() {
        ...
        let by_entity = db
            .list_audits_by_entity("projection", "proj-001")
    ...
    fn registry_connection_crud() {
        let db = test_db();
        let conn = db
            .insert_registry(&NewRegistryConnection {
